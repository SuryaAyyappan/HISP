...
const ScrollNumber = ({ targetNumber, unit, title }) => {
  ...
  return (
    <div ref={ref} className='font-afacad-flux text-center'>
      <motion.h1
        className='text-transparent bg-gradient-to-tr from-blue-800 to-black bg-clip-text text-6xl md:text-8xl'
        initial={{ opacity: 0 }}
        animate={{ opacity: inView ? 1 : 0 }}
        transition={{ duration: 1 }}>
        {count}
      </motion.h1>
      <h2 className='text-black text-3xl font-mono'>{unit}</h2>
      <h2 className='text-black font-raleway'>{title}</h2>
    </div>
  );
};
